...
const Event = ({ blok, history, slug, join_community, uid}) => {
  ...
  if (!history) {
    ...
    add_to_calc = <a href={url} class="btn active" style={{background: '#d9137b', borderColor:'d9137b', fontWeight: "600", fontFamily: "'Poppins', sans-serif"}}  target="_blank" onClick={() => mixpanel.track('EventAddToCal', { title: event.title })}>Add to Calendar</a>
    
    if ((event.register) && (event.register != "")) {
      register = <a href={event.register} class="btn active" style={{
        background: '#d9137b',
        borderColor: 'd9137b',
        fontWeight: "600",
        fontFamily: "'Poppins', sans-serif"
      }} target="_blank" onClick={() => mixpanel.track('EventRegister', { title: event.title })}>Register</a>;
    }
  }
